...
class MyDDPPlugin(DDPPlugin):
    def init_ddp_connection(self, global_rank=None, world_size=None) -> None:
        ...
        print('[WANGTAN DEBUG] world size: {}'.format(int(os.environ["WORLD_SIZE"])))
        print('[WANGTAN DEBUG] rank: {}'.format(int(os.environ["RANK"])))
...
def main(_config):
    ...
    os.environ["NODE_RANK"] = str(os.environ["NODE_RANK"])
