...
def config():
    ...
    eval_task = None
    eval_root = None
    eval_config = None # if use the config of training
    test_after_train = False

    # our pair loss
    pairloss = None
    pairloss_text_side = False
    pairloss_img_side = False
    pairloss_group_side = False
    pairloss_pos_reg = False

    pairloss_norm = False # normalize the pair loss (range from 0 -> 1)
    pairloss_weight = 1.0
    pairloss_margin = 0.0
    pairloss_margin_v1 = 0.0
    pairloss_prop = 1.0
    sample_num = -1 # -1 represents to use full batch to calculate the pair loss
    score_activation = None
    score_pre_softmax = False
...
@ex.named_config
def task_pretrain_itc():
    exp_name = "mlm_itm_itc"
    datasets = ["coco", "vg", "sbu", "gcc"]
    # datasets = ["f30k"]
    loss_names = _loss_names({"itm": 0, "mlm": 0, "itc": 1})
    draw_false_image = 0
    batch_size = 4096
...
@ex.named_config
def task_pretrain_itc_itm():
    exp_name = "mlm_itm_itc"
    datasets = ["coco", "vg", "sbu", "gcc"]
    # datasets = ["f30k"]
    loss_names = _loss_names({"itm": 1, "mlm": 0, "itc": 1})
    draw_false_image = 0
    batch_size = 4096
...
@ex.named_config
def task_pretrain_itm():
    exp_name = "mlm_itm_itc"
    datasets = ["coco", "vg", "sbu", "gcc"]
    # datasets = ["f30k"]
    loss_names = _loss_names({"itm": 1, "mlm": 0, "itc": 0})
    draw_false_image = 0
    batch_size = 4096
...
@ex.named_config
def task_finetune_irtr_itm_coco_hardneg():
    exp_name = "finetune_irtr_itm_coco"
    datasets = ["coco"]
...
@ex.named_config
def task_finetune_irtr_itm_f30k_hardneg():
    exp_name = "finetune_irtr_itm_f30k"
    datasets = ["f30k"]
    loss_names = _loss_names({"itm": 1, "itc": 1})
    draw_false_image = 1
...
@ex.named_config
def task_finetune_irtr_itm_f30k_coco():
    exp_name = "finetune_irtr_itm_f30k_mscoco"
    datasets = ["f30k", "coco"]
    loss_names = _loss_names({"itm": 1})
    draw_false_image = 1
    batch_size = 1024
...
@ex.named_config
def task_finetune_irtr_itm_f30k_coco_hardneg():
    exp_name = "finetune_irtr_itm_f30k_mscoco_hardneg"
    datasets = ["f30k", "coco"]
    loss_names = _loss_names({"itm": 1, "itc": 1})
    draw_false_image = 1
    batch_size = 1024
...
@ex.named_config
def task_finetune_irtr_itm_4m():
    exp_name = "finetune_irtr_itm_4m"
    datasets = ["coco", "vg", "sbu", "gcc"]
    loss_names = _loss_names({"itm": 1})
    draw_false_image = 1
    batch_size = 1024
...
@ex.named_config
def task_finetune_irtr_itm_4m_hardneg():
    exp_name = "finetune_irtr_itm_4m_hardneg"
    datasets = ["coco", "vg", "sbu", "gcc"]
    loss_names = _loss_names({"itm": 1, "itc": 1})
    draw_false_image = 1
    batch_size = 1024
