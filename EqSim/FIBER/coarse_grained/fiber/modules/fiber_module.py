...
class FIBERTransformerSS(pl.LightningModule):
    def __init__(self, config, logger=None):
        ...
        # our loss criterion
        self.loss_func = objectives.pairloss_criterion(text_side=config["pairloss_text_side"], img_side=config["pairloss_img_side"],
                                                       group_side=config["pairloss_group_side"], pos_reg=config["pairloss_pos_reg"],
                                                       norm=config["pairloss_norm"], weight=config["pairloss_weight"], margin=config["pairloss_margin"])
    ...
    def forward(self, batch, benchmark_test=None):
        ret = dict()

        if benchmark_test == 'itm':
            ret.update(self.infer(batch, mask_text=False, mask_image=False))
            return ret

        if benchmark_test == 'itc':
            ret_img, ret_txt = {}, {}
            ret_img.update(self.infer(batch, mask_image=False, mask_text=False, image_only=True))
            ret_txt.update(self.infer(batch, mask_image=False, mask_text=False, text_only=True))
            return ret_img, ret_txt
        ...
        if "itm" in self.current_tasks:
            if "itc" in self.current_tasks:
                ret.update(objectives.compute_itm_hardneg(self, batch, ret["itc_i2t_logits"], ret["itc_t2i_logits"], ret["itc_logit_scale"], pairloss=self.hparams.config["pairloss"]))
            else:
    ...
    def test_epoch_end(self, outs):
        ...
        if self.hparams.config["loss_names"]["vqa"] > 0:
            objectives.vqa_test_wrapup(outs, model_name, log_dir=self.config["log_dir"], exp_name=self.config["exp_name"])
        ...
        fiber_utils.epoch_wrapup(self, logger=self.logger_txt)
