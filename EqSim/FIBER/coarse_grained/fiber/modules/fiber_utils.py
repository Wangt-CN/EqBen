...
def epoch_wrapup(pl_module, logger=None):
    ...
    if pl_module.hparams.config["get_recall_metric"] and not pl_module.training:
        ...
        if logger is not None:
            logger.info('From {}'.format(pl_module.hparams.config["load_path"].split("/")[-1]))
            logger.info('================ IRTR Results | ITC {} ================='.format(pl_module.hparams.config["get_recall_metric_itc"]))
            logger.info('IR1:{}  IR5:{}  IR10:{}  |  TR1:{}  TR5:{}  TR10:{}'.format(ir_r1.item(), ir_r5.item(), ir_r10.item(), tr_r1.item(), tr_r5.item(), tr_r10.item()))
            logger.info(pl_module.global_step)
        else:
            print('From {}'.format(pl_module.hparams.config["load_path"].split("/")[-1]))
